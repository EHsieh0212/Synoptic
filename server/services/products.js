...
const getProductsByKeyword = async (keyword) => {
  ...
  const mainProductsVariants =
    await variantsRepositoryInstance.findVariantsByProductIds(productIds);
  const productsWithVariants = await _combineProductWithVariant(
    mainProducts,
    mainProductsVariants,
  );
...
const getProductsByCategory = async (category, pageNum = 0) => {
  ...
  const productsRepositoryInstance = productsRepository();
  const variantsRepositoryInstance = variantsRepository();
  const { count, ids } = await productsRepositoryInstance.findIdsByCategoryAndPage(
    category,
    pageNum,
    offset,
    PAGE_SIZE,
