...
const functions = [
  ...
  },
  {
    name: 'search',
    description:
      'search queries products by their title and returns a list of product titles and their ids',
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string' },
...
async function recommendationByChatbot(userQuestion) {
    ...
    messages: [
      {
        role: 'system',
        content:
          'Please use our Synoptic product database, which you can access using functions to answer the following questions.',
      },
      {
        role: 'user',
  ...
  const theresult = db
    .filter(
      (item) =>
        result.toLowerCase().includes(item.title.toLowerCase()) &&
        !userQuestion.toLowerCase().includes(item.title.toLowerCase()),
    )
